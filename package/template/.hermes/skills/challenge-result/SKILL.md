...
Work read-only. Try to falsify the claimed behavior with an independent check, adversarial scenario or invariant. Do not change code, intent, criteria, status or acceptance.

Record a reproducible counterexample, evidence gap or explicit limit of the search in `product-memory/evidence.md` and `telemetry/run-log.md`. An opinion without evidence does not change the outcome.
