#!/bin/bash
# Canonical local proof for P0-05 (G1 + G5).
set -u -o pipefail
...
WORK="${TMPDIR:-/tmp}/agent-native-pdlc-p0-05"
DERIVED_DATA="$WORK/DerivedData"
TEST_RESULT_BUNDLE="$RUN_DIR/BudgetApp-P0-05.xcresult"
TEST_LOG="$WORK/xcodebuild-test.log"
XCRESULT_FILES_MANIFEST="$RUN_DIR/BudgetApp-P0-05.xcresult.files.sha256"
...
g1_paths = [
    "testbeds/budget-ios/run-p0-05-verification.sh",
...
payload = {
    "schema_version": 1, "run_id": run_id, "outcome": "P0-05",
    "created_at_utc": started,
    "canonical_command": "./testbeds/budget-ios/run-p0-05-verification.sh",
    "repository_runner": "testbeds/budget-ios/run-p0-05-verification.sh",
    ...
    "scopes": {"G1": {"inputs": [entry(path) for path in g1_paths]}, "G5": {"inputs": [entry(path) for path in g4_paths]}},
}
...
xcodebuild -project "$PROJECT" -scheme BudgetApp -destination "$DESTINATION" \
  ...
  -only-testing:BudgetAppTests/BudgetOperationTests/testP005OperationsAndStatisticsSurviveApplicationRestart \
  -only-testing:BudgetAppUITests/BudgetAppUITests/testP005OperationsAndStatisticsSurviveApplicationRestart \
  test >"$TEST_LOG" 2>&1
TEST_STATUS=$?
...
G5_STATUS="FAIL"
[ "$TEST_STATUS" -eq 0 ] && [ "$SCREENSHOT_COUNT" -ge 1 ] && G5_STATUS="PASS"
OVERALL_STATUS="FAIL"
[ "$G1_STATUS" = "PASS" ] && [ "$G5_STATUS" = "PASS" ] && OVERALL_STATUS="PASS"

{
  printf '%s\n' '# P0-05 automatic verification' '' 'report_version: 1' "run_id: $RUN_ID" 'Generated by: `./testbeds/budget-ios/run-p0-05-verification.sh`' "Started (UTC): $START" "Repository commit: $COMMIT" "Environment: $XCODE_VERSION; $SWIFT_VERSION" "Simulator: $DEVICE_NAME ($DEVICE_ID)" ''
  printf '%s\n' '## Contract' '' '- Canonical command: `./testbeds/budget-ios/run-p0-05-verification.sh`' '- Repository runner: `testbeds/budget-ios/run-p0-05-verification.sh`' "- Manifest: $MANIFEST" "- Versioned report: $REPORT" "- Versioned proof-record: $PROOF_RECORD" "- Xcode result bundle: $TEST_RESULT_BUNDLE" "- .xcresult files manifest: $XCRESULT_FILES_MANIFEST" "- .xcresult digest: $XCRESULT_DIGEST" ''
  printf '%s\n' '## G1 — сборка и запуск в iOS Simulator' '' '- gate_id: G1' '- criterion: приложение собирается и запускается в iOS Simulator' '- expected: selected P0-05 tests complete and create result bundle' "- observed: xcodebuild_test=$TEST_STATUS, result_bundle=$(if [ -d "$TEST_RESULT_BUNDLE" ]; then printf 'present'; else printf 'missing'; fi)" "- run_status: $G1_STATUS" ''
  printf '%s\n' '## G5 — восстановление после перезапуска' '' '- gate_id: G5' '- criterion: operations, selected-period totals and category breakdown survive application terminate and relaunch' '- fixture: income 1000 ₽ / Продукты; expense 300 ₽ / Транспорт; outside-period expense 200 ₽' '- expected: unit and UI P0-05 checks pass, with one reviewable after-restart screenshot attachment' "- observed: xcodebuild_test=$TEST_STATUS, screenshot_attachments=$SCREENSHOT_COUNT, attachment_export=$ATTACHMENT_EXPORT_STATUS" "- run_status: $G5_STATUS" ''
  printf '%s\n' '### Reviewable XCUITest artifacts' '' '- required_screenshot_attachments: p0-05-after-restart' "- observed_screenshot_attachment_files: $SCREENSHOT_COUNT" "- screenshot_attachment_status: $(if [ "$SCREENSHOT_COUNT" -ge 1 ]; then printf 'PASS'; else printf 'FAIL'; fi)" '- assertions: stored in Xcode result bundle and versioned test summary' '- Computer Use: not required for this automated proof' '' "## Итог: $OVERALL_STATUS" ''
...
python3 - "$PROOF_RECORD" "$RUN_ID" "$MANIFEST" "$MANIFEST_SHA256" "$REPORT" "$REPORT_SHA256" "$TEST_RESULT_BUNDLE" "$XCRESULT_FILES_MANIFEST" "$XCRESULT_DIGEST" "$SCREENSHOT_COUNT" "$ATTACHMENT_EXPORT_STATUS" "$TEST_STATUS" "$TEST_DURATION_SECONDS" "$G1_STATUS" "$G5_STATUS" "$OVERALL_STATUS" <<'PY'
import json, sys
from pathlib import Path
proof_path = Path(sys.argv[1])
(run_id, manifest, manifest_sha256, report, report_sha256, result_bundle, result_manifest, result_digest, screenshot_count, attachment_export_status, test_status, duration, g1, g5, overall) = sys.argv[2:]
payload = {
    "proof_record_version": 1, "run_id": run_id, "outcome": "P0-05",
    "manifest": {"path": manifest, "sha256": manifest_sha256},
    "report": {"path": report, "sha256": report_sha256},
    "gate_results": {"G1": g1, "G5": g5}, "overall_result": overall,
    "ui_evidence": {"xcodebuild_test": {"status": int(test_status), "duration_seconds": int(duration), "result_bundle": result_bundle}, "xcresult_files_manifest": {"path": result_manifest, "digest_sha256": result_digest}, "screenshot_attachments": {"required": ["p0-05-after-restart"], "observed_file_count": int(screenshot_count), "attachment_export_status": int(attachment_export_status), "status": "PASS" if int(screenshot_count) >= 1 else "FAIL"}, "computer_use_required": False},
