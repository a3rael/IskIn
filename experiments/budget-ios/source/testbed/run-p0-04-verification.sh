#!/bin/bash
# Canonical local proof for P0-04 (G1 + G4).
set -u -o pipefail
...
WORK="${TMPDIR:-/tmp}/agent-native-pdlc-p0-04"
DERIVED_DATA="$WORK/DerivedData"
TEST_RESULT_BUNDLE="$RUN_DIR/BudgetApp-P0-04.xcresult"
TEST_LOG="$WORK/xcodebuild-test.log"
XCRESULT_FILES_MANIFEST="$RUN_DIR/BudgetApp-P0-04.xcresult.files.sha256"
...
g1_paths = [
    "testbeds/budget-ios/run-p0-04-verification.sh",
...
payload = {
    "schema_version": 1, "run_id": run_id, "outcome": "P0-04",
    "created_at_utc": started,
    "canonical_command": "./testbeds/budget-ios/run-p0-04-verification.sh",
    "repository_runner": "testbeds/budget-ios/run-p0-04-verification.sh",
...
xcodebuild -project "$PROJECT" -scheme BudgetApp -destination "$DESTINATION" \
  ...
  -only-testing:BudgetAppTests/BudgetOperationTests/testP004ChangingAndDeletingOperationsRecalculatesStatistics \
  -only-testing:BudgetAppUITests/BudgetAppUITests/testP004ChangingAndDeletingRecalculatesSelectedPeriodStatistics \
  test >"$TEST_LOG" 2>&1
TEST_STATUS=$?
...
G4_STATUS="FAIL"
[ "$TEST_STATUS" -eq 0 ] && [ "$SCREENSHOT_COUNT" -ge 3 ] && G4_STATUS="PASS"
OVERALL_STATUS="FAIL"
[ "$G1_STATUS" = "PASS" ] && [ "$G4_STATUS" = "PASS" ] && OVERALL_STATUS="PASS"

{
  printf '%s\n' '# P0-04 automatic verification' '' 'report_version: 1' "run_id: $RUN_ID" 'Generated by: `./testbeds/budget-ios/run-p0-04-verification.sh`' "Started (UTC): $START" "Repository commit: $COMMIT" "Environment: $XCODE_VERSION; $SWIFT_VERSION" "Simulator: $DEVICE_NAME ($DEVICE_ID)" ''
  printf '%s\n' '## Contract' '' '- Canonical command: `./testbeds/budget-ios/run-p0-04-verification.sh`' '- Repository runner: `testbeds/budget-ios/run-p0-04-verification.sh`' "- Manifest: $MANIFEST" "- Versioned report: $REPORT" "- Versioned proof-record: $PROOF_RECORD" "- Xcode result bundle: $TEST_RESULT_BUNDLE" "- .xcresult files manifest: $XCRESULT_FILES_MANIFEST" "- .xcresult digest: $XCRESULT_DIGEST" ''
  printf '%s\n' '## G1 — сборка и запуск в iOS Simulator' '' '- gate_id: G1' '- criterion: приложение собирается и запускается в iOS Simulator' '- expected: selected P0-04 tests complete and create result bundle' "- observed: xcodebuild_test=$TEST_STATUS, result_bundle=$(if [ -d "$TEST_RESULT_BUNDLE" ]; then printf 'present'; else printf 'missing'; fi)" "- run_status: $G1_STATUS" ''
  printf '%s\n' '## G4 — статистика за период и по категориям' '' '- gate_id: G4' '- criterion: selected period recalculates income, expense, balance and category breakdown after change and deletion' '- fixture: income 1000 ₽ / Продукты; expense 300 ₽ / Транспорт; outside-period expense 200 ₽' '- expected: unit and UI P0-04 checks pass, with three reviewable screenshot attachments for initial, changed and deleted states' "- observed: xcodebuild_test=$TEST_STATUS, screenshot_attachments=$SCREENSHOT_COUNT, attachment_export=$ATTACHMENT_EXPORT_STATUS" "- run_status: $G4_STATUS" ''
  printf '%s\n' '### Reviewable XCUITest artifacts' '' '- required_screenshot_attachments: p0-04-initial-statistics, p0-04-after-change, p0-04-after-delete' "- observed_screenshot_attachment_files: $SCREENSHOT_COUNT" "- screenshot_attachment_status: $(if [ "$SCREENSHOT_COUNT" -ge 3 ]; then printf 'PASS'; else printf 'FAIL'; fi)" '- assertions: stored in Xcode result bundle and versioned test summary' '- Computer Use: not required for this automated proof' '' "## Итог: $OVERALL_STATUS" ''
...
payload = {
    "proof_record_version": 1, "run_id": run_id, "outcome": "P0-04",
    ...
    "ui_evidence": {"xcodebuild_test": {"status": int(test_status), "duration_seconds": int(duration), "result_bundle": result_bundle}, "xcresult_files_manifest": {"path": result_manifest, "digest_sha256": result_digest}, "screenshot_attachments": {"required": ["p0-04-initial-statistics", "p0-04-after-change", "p0-04-after-delete"], "observed_file_count": int(screenshot_count), "attachment_export_status": int(attachment_export_status), "status": "PASS" if int(screenshot_count) >= 3 else "FAIL"}, "computer_use_required": False},
