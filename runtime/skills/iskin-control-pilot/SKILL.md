...
2. If the repository has no `HEAD`, run `python3 .iskin/policy_gate.py --action bootstrap_checkpoint`. Continue only when exit is `0`, status is `DISCOVERY_ALLOWED`, reason includes `INITIAL_BASELINE_UNCOMMITTED`, and `bootstrap_checkpoint` is in `allowed_actions`. Re-read the gate JSON, `git diff --cached --name-only --no-renames`, unstaged/untracked status, and `git diff --cached --check`; create only the local technical initial commit. The gate must not stage or commit. If bootstrap is denied, do not commit and report the blocker. After the commit rerun the gate and continue into discovery; bootstrap is not approval and does not authorize product work.
...
9. Reconcile outcome/lifecycle, evidence, telemetry, approval state, authority boundaries, human interventions, and next permitted action after a meaningful cycle.
