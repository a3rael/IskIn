...
7. Before a lifecycle or checkpoint commit, run `python3 .iskin/policy_gate.py --action checkpoint`; require exit `0` and the action-specific allowed status. A non-zero result blocks the commit. Avoid repeating current evidence without a documented reason.
