...
5. With write authority, store only reproducible findings in the canonical evidence and telemetry locations. If the finding changes lifecycle status, append a versioned lifecycle event and use `lifecycle_checkpoint`; never rewrite an old event or alter immutable package content. With read-only authority, return the finding, scope, and remaining uncertainty in the response without writing files.
