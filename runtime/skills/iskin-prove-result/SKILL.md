...
Use only for an IskIn outcome with approved applicable gates and a defined evidence scope. Otherwise return `not applicable` without changing files.

## Procedure

1. Read the outcome, `process/quality-gates.md`, `process/evidence-provenance.md`, current evidence, and uncertainties.
...
6. Set `proved` only when every approved mandatory gate, canonical artifact, proof record, and current fingerprint requirement succeeds. Do not set `accepted` without a human decision or pre-approved rule.

## Authority boundary

This skill does not weaken gates, manufacture evidence, overwrite historical evidence, or treat transient output as canonical proof.
