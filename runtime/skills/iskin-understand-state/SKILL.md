...
3. Run the read-only executable gate from the project root: `python3 .iskin/policy_gate.py --action read_only_recovery`. Treat non-zero output or `PROCESS_BLOCKED` as a blocker; do not replace its JSON with an interpretation of Markdown.
...
8. For an approval event, verify package ID, checkpoint SHA, `package_displayed_in_previous_agent_turn: true`, the exact question and human response, actor, authorization, and byte equality of every package path against the checkpoint.
