...
4. If `HEAD` is absent, also run `python3 .iskin/policy_gate.py --action bootstrap_checkpoint`. Record whether the exact installation baseline is valid and whether `bootstrap_checkpoint` is allowed. Re-read staged scope and `git diff --cached --check`; do not stage or commit in this read-only skill. Hand an allowed bootstrap to `iskin-control-pilot`; hand a denial to the blocker report.
